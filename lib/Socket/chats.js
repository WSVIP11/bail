...
const makeChatsSocket = (config) => {
 ...
 const getBusinessProfile = async (jid) => {
    ...
    // 2. Ambil Node Profile
    const profileNode = (0, WABinary_1.getBinaryNodeChild)(results, 'business_profile');
    const profiles = (0, WABinary_1.getBinaryNodeChild)(profileNode, 'profile');

    if (!profiles || !profiles.content) return null;

    // --- PERBAIKAN UTAMA ADA DI SINI (FUNGSI EKSTRAKSI) ---
    const extractText = (content) => {
        try {
            if (!content) return null;

            // 1. Jika ini Buffer asli Node.js
            if (Buffer.isBuffer(content)) {
                return content.toString('utf-8');
            }

            // 2. Jika ini Uint8Array (sering terjadi di library crypto/baileys)
            if (content instanceof Uint8Array) {
                return Buffer.from(content).toString('utf-8');
            }

            // 3. Jika content berupa Array (kadang teks dipecah)
            if (Array.isArray(content)) {
                return content.map(item => extractText(item)).join('');
            }

            // 4. Jika format JSON Buffer (fallback untuk kasus copy-paste data)
            if (typeof content === 'object' && content.type === 'Buffer' && content.data) {
            ...
            console.error("Gagal extract text:", e);
            return null;
        }
    };

    // --- Helper untuk mengambil data ---
    const getTagContent = (tag) => {
        // Mencari node dengan tag tertentu
        const node = Array.isArray(profiles.content) 
            ? profiles.content.find(n => n && n.tag === tag)
            : null;
    ...
    // --- Logika Parsing Khusus ---

    // 1. Websites
    const websites = Array.isArray(profiles.content) 
        ? profiles.content
    ...
    // 2. Categories
    const categoriesNode = Array.isArray(profiles.content) 
        ? profiles.content.find(n => n.tag === 'categories') 
        : null;
        
    const categories = categoriesNode && Array.isArray(categoriesNode.content)
        ? categoriesNode.content.map(c => extractText(c.content)).filter(Boolean)
        : [];

    // 3. Business Hours
    const hoursNode = Array.isArray(profiles.content) 
        ? profiles.content.find(n => n.tag === 'business_hours') 
        : null;
        
    const business_hours = hoursNode && Array.isArray(hoursNode.content)
        ? hoursNode.content.map(h => ({
            day: h.attrs ? h.attrs.day_of_week : null,
            mode: h.attrs ? h.attrs.mode : null
        }))
        : [];

    // --- Return Data ---
    return {
        jid: profiles.attrs ? profiles.attrs.jid : jid,
        display_name: getAttr('biz_identity_info', 'display_name'),
        description: getTagContent('description') || "", // Default ke string kosong jika null
        address: getTagContent('address') || "",
        email: getTagContent('email') || "",
        websites: websites,
        categories: categories,
        phone_number: getAttr('biz_identity_info', 'phone_number') || "",
        location: {
            latitude: parseFloat(getTagContent('latitude') || "0"),
            longitude: parseFloat(getTagContent('longitude') || "0")
        },
        business_hours: {
            timezone: hoursNode && hoursNode.attrs ? hoursNode.attrs.timezone : null,
            schedule: business_hours
        },
        metadata: {
            member_since: getTagContent('member_since_text'),
            is_signed: getAttr('biz_identity_info', 'is_signed') === 'true',
            verified_level: getAttr('biz_identity_info', 'vlevel')
        }
    };
};
